...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ARION unit input in lnReqAmount, we generate the URI with ARION as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(ArionUnits::format(ArionUnits::ARION, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
