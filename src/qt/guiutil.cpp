...
{
    widget->setMaxLength(ArionAddressValidator::MaxAddressLength);
    widget->setValidator(new ArionAddressValidator(parent));
...
bool parseArionURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ArionUnits::parse(ArionUnits::ARION, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseArionURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parseArionURI(uriInstance, out);
}
