...
namespace {
    class CArionAddressVisitor : public boost::static_visitor<bool> {
    private:
        CArionAddress *addr;
    public:
        CArionAddressVisitor(CArionAddress *addrIn) : addr(addrIn) { }
...
bool CArionAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CArionAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CArionAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CArionAddressVisitor(this), dest);
}

bool CArionAddress::IsValid() const {
...
CTxDestination CArionAddress::Get() const {
    if (!IsValid())
...
bool CArionAddress::GetKeyID(CKeyID &keyID) const {
...
bool CArionAddress::IsScript() const {
